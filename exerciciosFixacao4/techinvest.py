...
if gastoMensal <= renda:
    ...
    print(
        f"{nome} de acordo com o seu perfil voce quer assumir um risco {nivelCoragem} assim faremos a simulacao em {ondeInvestir}\n"
    )

    while True:
        maisAlgo = float(
            input(
                "Voce tem algum valor a mais guardado que deseja adicionar na simulacao ? (0 pra nao): "
            )
        )
        if maisAlgo < 0:
    ...
    lucroTotal = montante - valorInvest
    print(
        f"\nEm {qtsAnos} anos em {ondeInvestir} voce vai ter um montante de R$ {montante:.2f} ganho de R$ {lucroTotal:.2f}\n"
    )
else:
    print("\nEmergencia financeira")
    print(
        "Seu gasto e maior que sua renda por isso nao recomendamos investimentos antes de ter um dinheiro sobrando mensalmente"
    )
